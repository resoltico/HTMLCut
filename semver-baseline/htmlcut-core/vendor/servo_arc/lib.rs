...
impl<T> Arc<T> {
    ...
    pub fn borrow_arc<'a>(&'a self) -> ArcBorrow<'a, T> {
        unsafe { ArcBorrow::from_inner_ptr(self.ptr()) }
    }
...
impl<H, T> Arc<HeaderSlice<H, T>> {
    ...
    /// then `alloc` must return an allocation that can be deallocated
        ...
        I: Iterator<Item = T>,
    {
        ...
        let (layout, offset) = layout.extend(array_layout).expect("Overflow");
        let p = unsafe {
            // Allocate the buffer.
            let buffer = alloc(layout);
            let p = ptr::NonNull::new(buffer)
                .unwrap_or_else(|| alloc::handle_alloc_error(layout))
            ...
            ptr::write(ptr::addr_of_mut!((*p.as_ptr()).count), count);
            #[cfg(feature = "track_alloc_size")]
            ptr::write(ptr::addr_of_mut!((*p.as_ptr()).alloc_size), layout.size());
...
pub struct ArcBorrow<'a, T: 'a> {
    ptr: ptr::NonNull<ArcInner<T>>,
    phantom: PhantomData<&'a ArcInner<T>>,
}
...
impl<'a, T> ArcBorrow<'a, T> {
        ...
        T: 'static,
    {
        ...
        let transient = mem::ManuallyDrop::new(Arc {
            p: self.ptr,
            phantom: PhantomData,
        });
