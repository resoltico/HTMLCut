...
/// Serializes one value with the frozen `stable_json_v2` profile.
pub fn stable_json_v2<T: Serialize>(value: &T) -> Result<String, ContractError> {
...
pub(super) fn digest_stable_json<T: Serialize>(value: &T) -> Result<String, ContractError> {
    let stable_json = stable_json_v2(value)?;
