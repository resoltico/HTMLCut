...
use super::super::shared::{
    ContractError, INTEROP_V2_PROFILE, RESULT_SCHEMA_NAME, RESULT_SCHEMA_VERSION,
...
impl InteropResult {
    /// Builds one successful extraction result with the v2 schema identity.
    pub fn new(
    ...
    fn validate_body(&self) -> Result<(), ContractError> {
        ...
        validate_sha256_hex(
            "source.prepared_source_digest_sha256",
            &self.source.prepared_source_digest_sha256,
        )?;
        ...
        let selected_match_count = selected_match_count(self.selected_matches.len())?;
        if selected_match_count != expected_selected_matches {
...
fn selected_match_count(count: usize) -> Result<u32, ContractError> {
    u32::try_from(count)
        .map_err(|_| ContractError::SelectedMatchCountOutOfRange { received: count })
}

#[cfg(test)]
pub(crate) fn selected_match_count_for_tests(count: usize) -> Result<u32, ContractError> {
    selected_match_count(count)
}
