...
/// Interop profile identifier for v2.
pub const INTEROP_V2_PROFILE: &str = "htmlcut-v2";
...
pub const ERROR_SCHEMA_VERSION: u32 = 4;
...
pub enum ContractError {
    ...
    /// One CSS selector could not be compiled by the maintained selector engine.
    #[error("css selector grammar is invalid")]
    InvalidCssSelector,
    /// One regex delimiter boundary could not be compiled.
    #[error("{boundary} regex grammar is invalid")]
    InvalidDelimiterRegex {
        /// Boundary that failed compilation.
        boundary: &'static str,
    },
    /// One preparation limit exceeded its published hard maximum.
    #[error("preparation limits exceed the supported maximum")]
    PreparationLimitExceeded,
    /// One exploration request exceeded HTMLCut's published hard maxima.
    #[error("exploration limits exceed the supported maximum")]
    ExplorationLimitExceeded,
    /// A v2 fixed-width value could not be represented on the current platform.
    #[error("{field} cannot be represented on the current platform")]
    PlatformRange {
        /// Public field that exceeded the current platform range.
        field: &'static str,
    },
    ...
    SelectionModeCountMismatch {
        ...
        selected_match_count: u32,
        /// Number of matches the selection mode requires.
        expected_selected_matches: u32,
        /// Total candidate count.
        candidate_count: u32,
    },
    /// A caller attempted to validate more selected matches than the v2 wire count can encode.
    #[error("selected match count exceeds the v2 wire range: {received}")]
    SelectedMatchCountOutOfRange {
        /// Number of selected matches held by the caller.
        received: usize,
    },
