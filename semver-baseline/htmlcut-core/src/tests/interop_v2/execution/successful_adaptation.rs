...
fn successful_adaptation_projects_selector_results_and_selection_modes() {
    ...
    let adapted_text = v2::adapt_successful_extraction_for_tests(
        &selector_source
            .clone()
    ...
    let adapted_comparison_text = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &selector_plan(),
        successful_selector_extraction(
            vec![ExtractionMatch {
    ...
    let adapted_comparison_structured = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &Plan::new(
    ...
    let adapted_inner = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &Plan::new(
            PlanStrategy::css_selector(css_selector("article")),
            Selection::single(),
    ...
    let adapted_outer = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &Plan::new(
            PlanStrategy::css_selector(css_selector("article")),
            Selection::single(),
    ...
    let adapted_all = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &Plan::new(
            PlanStrategy::css_selector(css_selector("article")),
            Selection::all(),
    ...
    let adapted_projection_error = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &selector_plan(),
        successful_selector_extraction(
            vec![ExtractionMatch {
    ...
    let adapted_url_error = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &selector_plan(),
    ...
    let no_match_adapter_error = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &selector_plan(),
    ...
    let multi_match_adapter_error = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &selector_plan(),
        successful_selector_extraction(
            vec![selector_core_match(1, 1, 2), selector_core_match(2, 2, 2)],
...
#[test]
fn successful_adaptation_projects_delimiter_ranges_into_fixed_width_v2_evidence() {
    let plan = Plan::new(
    ...
    let adapted = v2::adapt_successful_extraction_for_tests(
        &selector_source(),
        &plan,
        successful_selector_extraction(vec![delimiter_match()], 1, None),
    )
    .expect("delimiter projection");

    assert_eq!(
        adapted.selected_matches[0].output_value,
        json!("<article>Hello</article>")
    );
    assert!(matches!(
        adapted.selected_matches[0].metadata,
