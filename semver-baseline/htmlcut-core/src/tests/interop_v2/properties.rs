...
use crate::interop::v2::{
    ...
    ErrorCode, InteropAdapterFailure, InteropDiagnostic, InteropDiagnosticCode,
    InteropDiagnosticLevel, InteropError, InteropErrorDetail, InteropResult, Output, Plan,
    PlanStrategy, RegexFlag, Rendering, ResultExecution, ResultSource, SelectedMatch,
    SelectedMatchMetadata, Selection, SelectionMode, StrategyKind, TextWhitespace, stable_json_v2,
};
...
impl CaseGenerator {
    ...
    fn selection(&mut self) -> Selection {
        match self.bounded(4) {
            ...
            1 => Selection::first(),
            2 => Selection::nth(
                NonZeroU32::new(u32::try_from(self.bounded(4) + 1).expect("small index"))
    ...
    fn range(&mut self) -> ByteRange {
        ...
        ByteRange {
            start: u64::try_from(start).expect("small start"),
            end: u64::try_from(start + self.bounded(16)).expect("small end"),
        }
    }
    ...
    fn selected_matches(
        ...
        match selection_mode {
            SelectionMode::Single | SelectionMode::First => vec![self.selected_match(
                ...
                output,
                candidate_count,
                NonZeroU32::new(1).expect("non-zero index"),
            )],
            SelectionMode::Nth => {
                let candidate_index = NonZeroU32::new(
                    u32::try_from(self.bounded(candidate_count as usize) + 1).expect("small index"),
                )
                .expect("non-zero index");
            ...
            SelectionMode::All => (1..=candidate_count)
                .map(|index| {
                    self.selected_match(
                        ...
                        output,
                        candidate_count,
                        NonZeroU32::new(index).expect("non-zero index"),
                    )
                })
                .collect(),
        }
    ...
    fn interop_result(&mut self) -> InteropResult {
        ...
        let candidate_count = u32::try_from(self.bounded(4) + 1).expect("small count");
...
#[test]
fn stable_json_v2_matches_recursive_canonical_serializer() {
    let mut generator = CaseGenerator::new(0x5172_0a9d_b3cc_41e3);

    for _ in 0..CASES {
        let value = generator.json_value(3);
        let stable = stable_json_v2(&value).expect("stable JSON should serialize");
