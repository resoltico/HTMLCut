...
pub fn cli_operation_help_document(operation_id: OperationId) -> Option<CliHelpDocument> {
    ...
    let overview_lines = match operation_id {
        OperationId::SourceInspect => vec![
            operation_descriptor(operation_id).description.to_owned(),
    ...
    Some(CliHelpDocument {
        ...
        examples: cli_operation_contract(operation_id)
...
pub(crate) fn cli_help_catalog_validation_errors() -> Vec<String> {
    ...
    if cli_aux_command_catalog().is_empty() {
        errors.push("cli_aux_command_catalog() is empty".to_owned());
    }

    for descriptor in cli_aux_command_catalog() {
    ...
    for descriptor in operation_catalog() {
        let help = cli_operation_help_document(descriptor.id);
        match (descriptor.cli_surface.is_some(), help.is_some()) {
    ...
    if cli_root_help_document().examples.is_empty() {
