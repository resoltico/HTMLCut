...
if ! jq -e '
    ...
    def artifact_parts:
        capture("^cargo-mutants-shard-(?<index>[0-9]+)-of-(?<total>[1-9][0-9]*)$")
        | {index: (.index | tonumber), total: (.total | tonumber)};
    ...
    | type == "array"
      and $shard_count > 0
      and ([$shards[].selector] | unique | length) == $shard_count
      ...
      and all($shards[];
          ...
          | $selector == $artifact
            and $selector.total == $shard_count
      )
      and ([
          $shards[].selector
          | selector_parts
          | .index
      ] | sort) == [range(0; $shard_count)]
' <<< "$shard_plan_json" >/dev/null; then
    echo "invalid mutation shard plan" >&2
    exit 1
fi

scratch_dir="$(mktemp -d "${TMPDIR:-/tmp}/htmlcut-mutants-summary.XXXXXX")"
cleanup_scratch_dir() {
...
while IFS=$'\t' read -r selector artifact_name; do
