...
fn builder_validation_edges_surface_cli_errors_and_invalid_inputs() {
    ...
    assert!(
        PreparedExtraction::from_slice(SliceArgs {
            ...
            output: default_slice_extract_output(CliSliceValueMode::Text),
            file_write: default_file_write_args(),
        })
        .is_err()
    );
    ...
    assert!(
        PreparedExtraction::from_slice(SliceArgs {
            ...
            output: default_slice_extract_output(CliSliceValueMode::Attribute),
            file_write: default_file_write_args(),
        })
        .is_err()
    );
...
fn default_slice_extract_output(value: CliSliceValueMode) -> SliceExtractOutputArgs {
