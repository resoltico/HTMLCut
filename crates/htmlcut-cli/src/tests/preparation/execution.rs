...
fn execution_paths_cover_direct_success_and_failure_variants() {
    ...
    let too_large = parse_byte_size("18446744073709551615kib").expect_err("too large byte size");
    ...
    assert!(
        wrap_html_document(&one_structured_match_report)
            .expect("single structured html bundle")
            .contains("<pre>")
    );
    ...
    assert!(
        wrap_html_document(&multi_match_report)
            .expect("multi-match html bundle")
            .contains("data-match-index=\"2\"")
    );
    ...
    assert_eq!(
        render_match_as_html(&outer_html_match).expect("outer html render"),
