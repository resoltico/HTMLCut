...
use crate::interop::v2::{
    HtmlInput, InteropError, InteropResult, Plan, exact_plan_digest_sha256_for_tests,
    execute_one_shot_for_tests, stable_json_v2,
};
...
fn canonical_plan_json(case: &AcceptanceCase, plan: &Plan) -> String {
    match case.expected_kind {
        ExpectedDocumentKind::PlanError => {
            stable_json_v2(plan).expect("invalid plan fixture stable json")
        }
...
fn update_fixtures() {
    ...
    for case in ACCEPTANCE_CASES {
        ...
        let plan: Plan = serde_json::from_str(&fixture_text(&plan_path))
            .expect("fixture plan must decode before regeneration");
        let new_plan_json = canonical_plan_json(case, &plan);
        fs::write(&plan_path, format!("{new_plan_json}\n")).expect("write plan");

        match case.expected_kind {
            ExpectedDocumentKind::Result => {
                let result = execute_one_shot_for_tests(&source, &plan).expect("execute plan");
            ...
            ExpectedDocumentKind::Error | ExpectedDocumentKind::PlanError => {
                let err =
                    execute_one_shot_for_tests(&source, &plan).expect_err("execute plan error");
...
#[test]
fn htmlcut_v2_acceptance_fixtures_are_canonical_and_deterministic() {
    for case in ACCEPTANCE_CASES {
        ...
        match case.expected_kind {
            ExpectedDocumentKind::Result => {
                ...
                let actual = execute_one_shot_for_tests(&source, &plan).expect("fixture result");
                let repeated =
                    execute_one_shot_for_tests(&source, &plan).expect("repeated fixture result");
            ...
            ExpectedDocumentKind::Error | ExpectedDocumentKind::PlanError => {
                ...
                let actual = execute_one_shot_for_tests(&source, &plan).expect_err("fixture error");
                let repeated =
                    execute_one_shot_for_tests(&source, &plan).expect_err("repeated fixture error");
