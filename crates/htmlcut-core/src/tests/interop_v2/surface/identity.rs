...
#[test]
fn stable_json_v2_sorts_object_keys_recursively() {
    let value = json!({
        "z": 1,
        "a": {
            "d": 4,
    ...
    let stable = stable_json_v2(&value).expect("stable json");
...
#[test]
fn prepared_document_retains_the_exact_input_base_url() {
    let base_url = http_url("https://example.com/docs/start.html");
    let document = prepare_document(
        HtmlInput::new("base-url-identity", "<article>Hello</article>")
            .expect("source input")
            .with_input_base_url(base_url.clone()),
    ...
    assert_eq!(document.input_base_url(), Some(&base_url));
}
...
fn html_input_extraction_identity_binds_complete_input_plan_and_semantics_version() {
    ...
    assert_eq!(
        identity,
        "826974d5a3b6a78ce0de07e8a5403b0572b40d38c1898ee9a7cc1ff79ffbbc7a"
    );
    assert_eq!(
        source
            .extraction_identity_sha256(&plan)
...
fn html_input_extraction_identity_binds_invalid_plans_for_diagnostics() {
    ...
    assert_eq!(
        identity,
        "29a6f710e53b447736515dee595f2ffa59050d2a378328a9e062c9c6714b4736"
    );
    assert_eq!(
        compile_plan(&plan)
