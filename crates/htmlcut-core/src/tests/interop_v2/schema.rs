...
use crate::interop::v2::DomCanonicalization;
use serde_json::json;
...
#[test]
fn interop_diagnostics_preserve_the_closed_source_preparation_classification() {
    let diagnostic = crate::Diagnostic {
        level: crate::DiagnosticLevel::Error,
        code: crate::DiagnosticCode::SourcePreparationFailed,
        message: "Preparation failed.".to_owned(),
        details: None,
    };
    let interop = InteropDiagnostic::from(&diagnostic);
    assert_eq!(interop.code, InteropDiagnosticCode::SourcePreparationFailed);
}
...
fn interop_plan_schema_enforces_h5_canonicalization_shape_and_relationships() {
    ...
    for output in [
        ...
        assert!(
            !interop_schema_accepts(v2::PLAN_SCHEMA_NAME, v2::PLAN_SCHEMA_VERSION, &document),
...
fn interop_result_schema_enforces_h5_evidence_boundaries_and_marks_the_runtime_boundary() {
    ...
    assert!(interop_schema_accepts(
        v2::RESULT_SCHEMA_NAME,
        v2::RESULT_SCHEMA_VERSION,
...
fn interop_error_and_result_schemas_publish_message_length_limits() {
    ...
    assert!(!interop_schema_accepts(
        v2::ERROR_SCHEMA_NAME,
        v2::ERROR_SCHEMA_VERSION,
        &error_document,
    ));
...
#[test]
fn interop_error_schema_rejects_legacy_and_unknown_detail_shapes() {
    let error = InteropError::new(
        TEST_PLAN_DIGEST_SHA256,
        ErrorCode::NoMatch,
    ...
    .with_computed_digest()
    .expect("valid error");
    let mut legacy = serde_json::to_value(&error).expect("error JSON");
    let fields = legacy.as_object_mut().expect("error object");
    let detail = fields.remove("detail").expect("current detail");
    fields.insert("details".to_owned(), detail);
    assert!(!interop_schema_accepts(
        v2::ERROR_SCHEMA_NAME,
        v2::ERROR_SCHEMA_VERSION,
        &legacy,
    ));
    assert!(serde_json::from_value::<InteropError>(legacy).is_err());

    let mut unknown_detail = serde_json::to_value(&error).expect("error JSON");
    unknown_detail["detail"]["unexpected"] = json!(true);
    assert!(!interop_schema_accepts(
        v2::ERROR_SCHEMA_NAME,
        v2::ERROR_SCHEMA_VERSION,
        &unknown_detail,
    ));
    assert!(serde_json::from_value::<InteropError>(unknown_detail).is_err());
}
...
fn interop_public_helpers_cover_selection_modes_and_html_input_paths() {
    ...
    assert_eq!(
        Selection::nth(NonZeroU32::new(3).expect("index")).mode(),
        SelectionMode::Nth
    );
...
fn interop_schema_identity_helpers_reject_name_version_and_profile_drift() {
    let name_error = v2::validate_schema_identity_for_tests(
        "wrong",
        v2::PLAN_SCHEMA_NAME,
        v2::PLAN_SCHEMA_VERSION,
        v2::PLAN_SCHEMA_VERSION,
        v2::INTEROP_V2_PROFILE,
        v2::INTEROP_V2_PROFILE,
    )
    ...
    let version_error = v2::validate_schema_identity_for_tests(
        v2::PLAN_SCHEMA_NAME,
        v2::PLAN_SCHEMA_NAME,
        99,
        v2::PLAN_SCHEMA_VERSION,
    ...
    let profile_error = v2::validate_schema_identity_for_tests(
        v2::PLAN_SCHEMA_NAME,
        v2::PLAN_SCHEMA_NAME,
        v2::PLAN_SCHEMA_VERSION,
        v2::PLAN_SCHEMA_VERSION,
