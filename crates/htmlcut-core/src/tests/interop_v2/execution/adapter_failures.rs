...
fn adapter_rejects_invalid_delimiter_shapes_and_unsupported_outputs() {
    ...
    let delimiter_missing_attributes_error = v2::project_structured_match_for_tests(
        &ExtractionMatch {
            value: json!({
    ...
    let delimiter_non_string_attribute_error = v2::project_structured_match_for_tests(
        &ExtractionMatch {
            value: json!({
                "selectedHtmlOutput": "<article>Hello</article>",
                "textOutput": "Hello",
    ...
    let selected_html_projection_error = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &Plan::new(
    ...
    let missing_attribute_error = v2::adapt_successful_extraction_for_tests(
        &selector_source,
        &Plan::new(
            PlanStrategy::css_selector(css_selector("article")),
            Selection::single(),
...
#[test]
fn result_validation_rejects_an_unrepresentable_selected_match_count_without_allocating_it() {
    if usize::BITS <= u32::BITS {
        return;
    }
    let beyond_v2_count = usize::try_from(u32::MAX).expect("u32 fits usize") + 1;
    assert!(matches!(
        v2::selected_match_count_for_tests(beyond_v2_count),
        Err(ContractError::SelectedMatchCountOutOfRange { received }) if received == beyond_v2_count
    ));
}
