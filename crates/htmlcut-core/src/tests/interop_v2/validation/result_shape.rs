...
fn interop_result_validation_rejects_zero_counts_range_drift_and_metadata_mismatch() {
    ...
    out_of_range.selected_matches[0].candidate_index = NonZeroU32::new(2).expect("index");
...
fn interop_stable_json_digest_helpers_cover_object_and_scalar_values() {
    let object_digest =
        v2::digest_stable_json_omitting_field_for_tests(&json!({"keep": 1, "drop": 2}), "drop")
            .expect("object digest");
    let expected_object_digest =
        v2::digest_stable_json_omitting_field_for_tests(&json!({"keep": 1}), "drop")
            .expect("object digest");
    assert_eq!(object_digest, expected_object_digest);

    let scalar_digest = v2::digest_stable_json_omitting_field_for_tests(&json!("value"), "drop")
        .expect("scalar digest");
    let expected_scalar_digest =
        v2::digest_stable_json_omitting_field_for_tests(&json!("value"), "other")
            .expect("scalar digest");
...
fn interop_validation_rejects_invalid_and_mismatched_digests_for_results_and_errors() {
    ...
    let invalid_source = ResultSource {
        prepared_source_digest_sha256: "not-a-digest".to_owned(),
        ..source.clone()
    };
    let mut invalid_source_result = InteropResult::new(
        execution.clone(),
        invalid_source,
        selector_selected_matches(),
        Vec::new(),
    );
    invalid_source_result.result_digest_sha256 = TEST_PLAN_DIGEST_SHA256.to_owned();
    assert!(matches!(
        invalid_source_result.validate(),
        Err(ContractError::InvalidDigest {
            field: "source.prepared_source_digest_sha256",
            ..
        })
    ));
