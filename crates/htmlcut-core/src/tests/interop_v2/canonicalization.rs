use super::*;
use crate::interop::v2::{DomCanonicalization, compile_plan, execute_one_shot_for_tests};
...
fn dom_canonicalization_selects_original_candidates_and_preserves_raw_evidence() {
    ...
    let baseline =
        execute_one_shot_for_tests(&source, &baseline_plan).expect("baseline extraction");
    let canonical =
        execute_one_shot_for_tests(&source, &canonical_plan).expect("canonical extraction");
...
fn dom_canonicalization_keeps_structured_output_raw() {
    ...
    let result =
        execute_one_shot_for_tests(&source(), &plan).expect("canonical structured extraction");
    let selected = result.selected_matches.first().expect("selected match");
...
fn dom_canonicalization_has_no_implicit_attribute_denylist() {
    ...
    let result = execute_one_shot_for_tests(&source(), &plan).expect("canonical extraction");
    let selected = result.selected_matches.first().expect("selected match");
...
fn dom_canonicalization_rejects_raw_outputs_without_a_comparison_projection() {
    for output in [
        ...
        let error = compile_plan(&plan).expect_err("raw output canonicalization must fail");
...
fn dom_canonicalization_rejects_removed_controls_non_css_strategies_and_ignored_measured_attributes()
 {
    ...
    let non_css_error = compile_plan(&non_css).expect_err("non-CSS canonicalization must fail");
    assert_eq!(non_css_error.error_code, ErrorCode::PlanInvalid);
    assert_eq!(non_css_error.detail, InteropErrorDetail::ContractViolation);
    ...
    let ignored_measured_attribute_error = compile_plan(&ignored_measured_attribute)
...
fn dom_canonicalization_strips_whitespace_nodes_only_from_comparison_text() {
    ...
    let result =
        execute_one_shot_for_tests(&source, &plan).expect("canonical whitespace extraction");
    let selected = result.selected_matches.first().expect("selected match");
...
fn dom_canonicalization_all_selection_keeps_each_raw_candidate_independent() {
    ...
    let result = execute_one_shot_for_tests(&source, &plan).expect("canonical all extraction");
