...
use crate::interop::v2::{
    self as v2, AttributeName, ByteRange, ContractError, CssSelectorText,
    DelimiterBoundaryRetention, DelimiterBoundaryText, DelimiterMode, DisplayedHttpUrl, ErrorCode,
    HtmlInput, HttpUrl, InteropAdapterFailure, InteropDiagnostic, InteropDiagnosticCode,
    InteropDiagnosticLevel, InteropError, InteropErrorDetail, InteropFinalizationRejection,
    InteropResult, Output, OutputKind, Plan, PlanStrategy, RegexFlag, Rendering, ResultExecution,
    ResultSource, SelectedMatch, SelectedMatchMetadata, Selection, SelectionMode,
    SelectorParseDetail, SelectorParseErrorClass, StrategyKind, TextWhitespace,
};
use crate::result::{
...
fn delimiter_plan() -> Plan {
    Plan::new(
        ...
        Selection::nth(NonZeroU32::new(2).expect("index")),
        Output::outer_html(),
...
fn selector_parse_detail(
    line: u64,
    column_utf16: u64,
    parse_error_class: SelectorParseErrorClass,
) -> SelectorParseDetail {
    SelectorParseDetail {
        line: NonZeroU64::new(line).expect("selector line"),
        column_utf16: NonZeroU64::new(column_utf16).expect("selector column"),
        parse_error_class,
    }
}
...
fn invalid_selector_interop_error(
...
fn selector_selected_match_with(candidate_count: u32, candidate_index: u32) -> SelectedMatch {
    let candidate_index = NonZeroU32::new(candidate_index).expect("candidate index");
    SelectedMatch {
        candidate_index,
...
fn delimiter_selected_match_with(candidate_count: u32, candidate_index: u32) -> SelectedMatch {
    let candidate_index = NonZeroU32::new(candidate_index).expect("candidate index");
    SelectedMatch {
        candidate_index,
