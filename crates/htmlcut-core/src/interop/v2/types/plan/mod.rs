...
use super::shared::{
    ContractError, INTEROP_V2_PROFILE, PLAN_SCHEMA_NAME, PLAN_SCHEMA_VERSION,
...
non_empty_string_type!(
    CssSelectorText,
    EmptyCssSelector,
    "Validated CSS selector text owned by htmlcut-v2."
);
non_empty_string_type!(
    DelimiterBoundaryText,
    EmptyDelimiterBoundary,
    "Validated delimiter boundary text owned by htmlcut-v2."
);

/// Strategy family available in v2.
...
/// Candidate selection mode available in v2.
#[derive(Clone, Copy, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
impl Selection {
    ...
    pub const fn nth(index: NonZeroU32) -> Self {
        Self::Nth { index }
    }
...
/// Output payload kind available in v2.
#[derive(Clone, Copy, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum OutputKind {
...
/// Text-whitespace mode available in v2.
#[derive(Clone, Copy, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Resource limits that can change one plan's observable result.
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct ExecutionBudget {
    /// Maximum selector-engine work units consumed by one execution.
    pub max_selector_work_units: NonZeroU32,
    /// Maximum candidates inspected before selection.
    pub max_candidates: NonZeroU32,
    /// Maximum selected matches emitted by one result.
    pub max_selected_matches: NonZeroU32,
    /// Maximum UTF-8 bytes in one selected output payload.
    pub max_match_output_bytes: NonZeroU32,
    /// Maximum UTF-8 bytes across all emitted output payloads.
    pub max_total_output_bytes: NonZeroU32,
}

impl Default for ExecutionBudget {
    fn default() -> Self {
        Self {
            max_selector_work_units: NonZeroU32::new(1_000_000).expect("non-zero constant"),
            max_candidates: NonZeroU32::new(100_000).expect("non-zero constant"),
            max_selected_matches: NonZeroU32::new(10_000).expect("non-zero constant"),
            max_match_output_bytes: NonZeroU32::new(8 * 1024 * 1024).expect("non-zero constant"),
            max_total_output_bytes: NonZeroU32::new(64 * 1024 * 1024).expect("non-zero constant"),
        }
    }
}
